...
import validatenewCar from '../validations/validateData';
...
const Car = {
    async create(req, res) {
        ...
        const checkIfExists = CarModel.carExists(req.body.owner, req.body);
        if (checkIfExists) {
            return res.status(400).send({
                status: 400,
    ...
    getSingleAd(req, res) {
        ...
        if (!car) {
            return res.status(404).send({
                status: 404,
                message: 'Oops! this car AD does not exist',
            });
        }
        return res.status(200).send({
            status: 200,
    ...
    updateAdvert(req, res) {
        ...
        if (!car) {
            return res.status(404).send({
                status: 404,
                message: 'Oops! The car AD you want to update doesn\'t exist',
            });
        }
        ...
        if (parseInt(userId, 10) !== parseInt(car.owner, 10) && !role) {
            return res.status(401).send({
                status: 401,
                message: 'Oh No! You do not have the permission to update this data',
            });
        }
    ...
    getCarsWithinPriceRange(req, res) {
        ...
        if (cars.length < 1) {
            return res.status(404).send({
                status: 404,
                message: 'Oops! There are no available cars within this range',
            });
        }
